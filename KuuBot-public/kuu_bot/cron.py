...
def start(send_message, agent=None):
    global _send_func, _agent, _running
...
def _random_chat(openid, today):
    ...
    try:
        ...
        result = _agent.chat([{
            "role": "user",
            "content": casual_note + "（这是系统主动触发的闲聊，不是回复消息。话题随机发挥，简短一些，控制在两三句话内，可以是吐槽、卖萌、问候、小段子）" + topic_hint
        }])
