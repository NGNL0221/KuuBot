...
TOOL_DEFS = [
    ...
    },
    {
        ...
        "function": {
            "name": "glob",
            "description": f"⚠️ 仅限本地文件搜索。搜索匹配的文件名。path 默认 {DEFAULT_DIR}",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "grep",
            "description": f"⚠️ 仅限本地文件搜索。在文件内容中搜索正则匹配。path 默认 {DEFAULT_DIR}",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "sticker",
            "description": "回复主人时必须调用此函数发送匹配情绪的表情包。根据你的回复内容选择最合适的标签",
            "parameters": {
                "type": "object",
                "properties": {
                    "tag": {
                        "type": "string",
                        "description": "表情标签，从以下选一个最匹配的: 震惊佩服(看到牛逼东西) 普通困惑(没听懂) 无语鄙夷(想挂网上) 点赞赞同(支持) 震惊变态(离人类远) 不服挑衅(蒸馍不服) 傻子挑衅(傻了吧唧) 失落(难过) 调戏挑衅(艹我啊) 忍怒生气(嘟嘴忍着) 不赖赞同(不赖) 啊困惑(啊？) 闷气诅咒(画圈圈) 害羞挡脸(用书挡脸) 贱笑拍照(举手机拍照) 贱笑(单纯贱笑) 中指鄙视(竖中指) 可爱困惑(可爱的困惑) 轻松开心(猫嘴笑)",
                    },
                },
                "required": ["tag"],
            },
    ...
    },
    {
        ...
        "function": {
            "name": "search",
            "description": "🔥 联网搜索。你的训练数据截止2025年已过时，任何需要查证事实的问题必须先调用此工具搜索，禁止凭训练记忆回答。查询实时数据、新闻、具体事实时必用",
            "parameters": {
                "type": "object",
                "properties": {
